...
from coding.part2.statistics import *
...
class txtdata:
    ...
    def update(self):
        ...
        today = update_day[1:4]
        l = lastdate.strip()
        last = datetime.strptime(l,"%m월 %d일")
        toda = datetime.strptime(today,"%m.%d")
        if toda<=last :
            return
        else:
            #txt 파일 상대경로 -> 절대경로
            ...
            f = open(path,'a')
            today = today.replace(".","월 ")
            today += "일"
            print(today)
            today_data = today+"\t"+today_definite+"\t"+today_treat+"\t"+today_death+"\t"
            today_data += str(total[4:].replace(",",""))+"\t"+str(definite.replace(",",""))+"\t"+str(death.replace(",",""))+"\n"
            f.write(today_data)
            f.close()
